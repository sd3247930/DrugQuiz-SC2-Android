...
- 验收：安卓版网页资源 **55/55** 通过；服务端版 pytest **47/47** + 浏览器 **60/60**；静态版 **80/80**（串行执行，零倒退）；
- 已产出 APK：`com.drugquiz.sc2`，versionCode **8** / versionName **1.5.2**（含答题卡矩阵、进度导出导入、我的笔记、Toast 修复、桌面图标与顶栏 Logo 去白边、四页英文副标题、状态栏浅色图标）；
...
### 5.1 App 内网页资源（`node verify_www.mjs`，**55 / 55 通过**）
...
| 静态单文件版 | `node tools/verify-ui.mjs` | **80 / 80 通过** |
